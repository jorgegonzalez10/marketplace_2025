...
#   end
Shipment.destory_all
Payment.destory_all
OrderItem.destory_all
Order.destroy_all
Address.destroy_all
...
products = Product.create!([
  ...
  { title: 'Tv', price: '500', published: true, category_id: categories[1].id, user_id: users[0].id }
  { title: 't-shirt', price: '20', published: true, category_id: categories[2].id, user_id: users[0].id }
])

adresses = Address.create!([
  {street: '5801 preston oaks rd', city: 'dallas', zip: '75254', country: 'usa', type: 'apartment', user_id: users[1].id }
  {street: '2340 creekview dr', city: 'waco', zip: '76711', country: 'usa', type: 'apartment', user_id: users[1].id }
])
...
order = Order.create!(
  user_id: users[1].id, address_id: Address.all.sample, total: CartItem.map { |item| item.price * item.quantity }.sum
)
